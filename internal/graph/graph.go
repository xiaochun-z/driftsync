...
type DriveItem struct {
	...
	ParentReference *struct{ Path string `json:"path"` } `json:"parentReference"`
...
func (c *Client) RootDelta(ctx context.Context, deltaLink string) (*DeltaResponse, error) {
	var u string
	if deltaLink != "" { u = deltaLink } else { u = c.Base + "/me/drive/root/delta" }
	...
	var d DeltaResponse
	if err := json.NewDecoder(resp.Body).Decode(&d); err != nil { return nil, err }
	return &d, nil
}
...
func (c *Client) DownloadTo(ctx context.Context, itemID, destPath string) error {
	...
	if dl == "" { return fmt.Errorf("no @microsoft.graph.downloadUrl on item %s", itemID) }
...
func (c *Client) createUploadSession(ctx context.Context, safePath string) (string, error) {
	...
	var out struct{ UploadURL string `json:"uploadUrl"` }
	if err := json.NewDecoder(resp.Body).Decode(&out); err != nil { return "", err }
	if out.UploadURL == "" { return "", fmt.Errorf("empty uploadUrl") }
