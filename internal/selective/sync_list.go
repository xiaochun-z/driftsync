...
func Load(path string) (*List, error) {
	...
	for sc.Scan() {
		...
		if r.Exclude {
			if r.Absolute {
				l.ExcludeAbs = append(l.ExcludeAbs, norm)
			} else {
				l.ExcludeAnywhere = append(l.ExcludeAnywhere, norm)
			}
		} else {
			if r.Absolute {
				l.IncludeAbs = append(l.IncludeAbs, norm)
			} else {
				l.IncludeAnywhere = append(l.IncludeAnywhere, norm)
			}
		}
	}
