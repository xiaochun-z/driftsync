...
import { GetConfig, SaveConfig, StartSync, StopSync, SelectDirectory, OpenDirectory, LocateFile, GetRemoteItems, StartOver, GetStartOverPaths } from '../wailsjs/go/main/App'
        ...
        <div v-else-if="currentTab === 'setup'" class="p-4 flex flex-col h-full absolute inset-0 pb-6">
          <div class="max-w-xl mx-auto w-full flex flex-col h-full">
            <div class="text-center mb-4 mt-2">
              <h2 class="text-xl font-bold text-white tracking-wide mb-1">Choose What To Sync</h2>
              <p class="text-sm text-slate-400">Select the files and folders you want to sync from your OneDrive. Unchecked items will be kept in the cloud but removed or ignored locally.</p>
            </div>

            <div class="flex-1 bg-slate-900/50 backdrop-blur-sm border border-slate-800/80 rounded-xl p-3 overflow-y-auto custom-scrollbar shadow-inner">
              <div v-if="loadingWizardNodes" class="h-full flex flex-col items-center justify-center gap-3">
                ...
                <span class="text-slate-400 font-medium">Loading your OneDrive contents...</span>
              </div>
              <div v-else class="space-y-2">
                <div class="flex justify-start mb-3 pl-8 pt-1">
            ...
            <div class="mt-4 pt-4 border-t border-slate-800/80">
              <button @click="completeSetup" class="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-lg text-sm font-bold transition-all shadow-md active:scale-95 bg-gradient-to-r from-indigo-500 to-blue-600 hover:from-indigo-400 hover:to-blue-500 text-white border border-indigo-400/50">
